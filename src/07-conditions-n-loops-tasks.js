...
function getSumBetweenNumbers(n1, n2) {
  ...
  const SumN1 = (second * (second + 1)) / 2;
  const SumN2 = ((first - 1) * first) / 2;
  return SumN1 - SumN2;
}
...
function doRectanglesOverlap(rect1, rect2) {
  ...
  if (rect1.left >= rect2Right || rect2.left >= rect1Right
      || rect1.top >= rect2Bottom || rect2.top >= rect1Bottom) {
    return false;
  }
  return true;
}
...
function isInsideCircle(circle, point) {
  ...
  return distance < circle.radius; // запушить утром
}
...
function isBracketsBalanced(str) {
  ...
  str.split('').map((element) => {
    if (element === '(' || element === '{' || element === '[' || element === '<') {
      ...
      if (
        (element === ']' && last === '[') || (element === '}' && last === '{') || (element === ')' && last === '(') || (element === '>' && last === '<')
      ) {
        stack.pop();
      } else {
        result = false;
...
function getMatrixProduct(m1, m2) {
  return m1.map((row, i) => m2[0]
    .map((_, j) => row.reduce((a, b, n) => a + m1[i][n] * m2[n][j], 0)));
}
